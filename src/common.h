...
std::string toHex(const void *input, uint64_t size);
...
class TeeOstream {
public:
  TeeOstream(std::string name = "/dev/null", const char *color = nullptr) : my_fstream(name), c(color) {}
...
public:
  ...
  void output() {
    ...
    for (auto it = mem.begin(); it != mem.end(); ++it) {
      const std::string &solution = it->first;
  ...
  static inline void count(const std::string &solution, const std::string &type, double acc = 1) {
#ifdef PROFILE
    count(solution + " " + type, acc);
#endif
  }
  
  static inline void count(const std::string &solution, double acc = 1) {
#ifdef PROFILE
    count_(solution, acc);
#endif
  }
  
  static inline void count_(const std::string &solution, double acc = 1) {
#ifdef PROFILE
    lock_guard lg(lock);
  ...
  inline static unordered_map<std::string, double> &assertExistence(const std::string &solution) {
    thread::id id = this_thread::get_id();
    unordered_map<std::string, double> &mem1 = clockers[id].back()->mem;
  ...
  static inline double getCount(const std::string &solution, const std::string &type) {
    return getCount(solution + " " + type);
  }
  
  static inline void countMax(const std::string &solution, const std::string &type, double number) {
#ifdef PROFILE
    countMax(solution + " " + type, number);
#endif
  }
  
  static inline void countMin(const std::string &solution, const std::string &type, double number) {
#ifdef PROFILE
    countMin(solution + " " + type, number);
#endif
  }
  
  static inline double getCount(const std::string &solution) {
#ifdef PROFILE
    lock_guard lg(lock);
  ...
  static inline void countMax(const std::string &solution, double number) {
#ifdef PROFILE
    lock_guard lg(lock);
  ...
  static inline void countMin(const std::string &solution, double number) {
#ifdef PROFILE
    lock_guard lg(lock);
    auto &mem1 = assertExistence(solution);
